<script lang="ts">
        ...
        let featuredChars: CharacterRecord[] = [];
        ...
        function doSinglePull(): PullResult[] | null {
                ...
                // Roll rarity
                const pityBefore = state.pity.count;
                ...
                // Apply cost
                gameStore.state.currencies.myTicket[ticketType] -= costResult.ticketsUsed;
                gameStore.state.currencies.vgems -= costResult.vgemsUsed;

                // Handle result
                state.stats.totalPulls++;
                const result = handlePullResult(character, state.characters, gameStore.state.currencies);

                // Update character data
                state.characters[character.slug] = result.updatedChar;

                // Track pull history
                if (!state.pullHistory[character.slug]) {
                        state.pullHistory[character.slug] = { firstPullDate: new Date().toISOString(), totalPulls: 0 };
                }
                state.pullHistory[character.slug].totalPulls++;

                // Quest progress
                gameStore.state.quests = incrementQuestProgress(gameStore.state.quests, 'daily', 'daily_pull_1', 1);
