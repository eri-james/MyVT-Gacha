<script lang="ts">
        ...
        function doSinglePull(): PullResult[] | null {
                ...
                // Apply E6 LiveCache overflow
                if (result.liveCacheGained > 0) {
                        gameStore.addCurrency('liveCache', result.liveCacheGained);
                }
                ...
                // Quest progress
                gameStore.setQuests(incrementQuestProgress(gameStore.state.quests, 'daily', 'daily_pull', 1));
                gameStore.setQuests(incrementQuestProgress(gameStore.state.quests, 'weekly', 'weekly_pull30', 1));
        ...
        function doMultiPull(): PullResult[] | null {
                ...
                // Quest progress (10 pulls count as 1 for daily, but count individually for weekly)
                gameStore.setQuests(incrementQuestProgress(gameStore.state.quests, 'daily', 'daily_pull', 1));
                gameStore.setQuests(incrementQuestProgress(gameStore.state.quests, 'weekly', 'weekly_pull30', 10));
...
<div class="animate-fade-in">
        ...
        <PageHint
                step="gachaHint"
                title="Gacha Pulls"
                body="Spend VGems or Blue Tickets to scout new VTubers. SSR/UR are rare but guaranteed at 90 pulls (pity). Featured banner has rate-up characters at 75% rate."
                icon="gacha"
        />
