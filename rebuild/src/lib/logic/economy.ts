...
import {
        ...
        DAILY_BASE_VGEMS, DAILY_INCREMENT_VGEMS, DAILY_CAP_VGEMS,
        DAILY_BASE_TICKETS, DAILY_CAP_TICKETS
...
export function getLoginRewards(streak: number): { vgems: number; tickets: number } {
        const vgems = Math.min(DAILY_CAP_VGEMS, DAILY_BASE_VGEMS + (streak - 1) * DAILY_INCREMENT_VGEMS);
        const tickets = Math.min(DAILY_CAP_TICKETS, DAILY_BASE_TICKETS + Math.floor((streak - 1) / 7));
